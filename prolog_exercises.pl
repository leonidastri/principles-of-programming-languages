...
insert((X,Y),[],[(X,Y)]).
insert((X,Y),[(H1,H2)|T],[(H1,H2)|W]):-
	Y<H2,
	insert((X,Y),T,W).
insert((X,Y),[(H1,H2)|T],[(X,Y)|W]):-
	Y>=H2,
	insert((H1,H2),T,W).
...
mylunch(1,Y,T):- seat(Y,T2), T is T2.
mylunch(2,Y,T):- seat(Y,T2), T is 7+T2.
mylunch(3,Y,T):- seat(Y,T2), T is T2.
mylunch(4,Y,T):- seat(Y,T2), T is 7+T2.
...
%reverse list
reverselist([],[]).
reverselist([X|Xs],Ys):- reverselist(Xs,Rs), append(Rs,[X],Ys).
...
createrow1(M,N,V,Sum,MS,MSV,[X|T]):-
	N>1,N =\= MS,
	NN is N - 1, 
	X is V, 
	NV is V+1, 
...
createrow1(M,N,_,Sum,MS,MSV,[X|T]):-
	N=:=1,
	NN is N - 1,
	X is M*MSV-Sum,
...
creatematrix1(M, N, Y, [X1|T1],[X2|T2]):-
	N>1,
	row1(M,Y,X1),
	head(X1,X2),
	NN is N-1,
...
sum([],0).
sum([X|T],Sum):-sum(T,Nsum), Sum is Nsum+X.
...
find([_|T],N,MS,R):- N =\= MS, N1 is N - 1, find(T,N1,MS,R).
find([X|_],N,MS,R):- N =:= MS, R is X.

lastrowfirst(N,Y,R):-sum(Y,Sum), MS is N-(N//2),find(Y,N,MS,MSV),R is N*MSV-Sum.
...
findlength( [], L,L ).
findlength( [_|T] , N , L ) :-N1 is N+1,findlength(T,N1,L).

%Check for last seat
checklast([X|_],F,L,M):-X = e, F is 1,M is L-1.
checklast([X|_],F,_,M):-X \= e,	F is 0, M is -1.

%check for first seat if last is taken
checkfirst([X|_], F, NF, _, M):-
	F =\= 1,
	X = e,
	NF is 1,
	M is 0.
checkfirst([X|_], F, NF, _, M):-
	...
	NF is 0,
	M is -1.
checkfirst([_|_], F, NF, PM, M):-
	F =:= 1,
	NF is 1,
	M is PM.

%Check starting from left to find two empty seats in a row and seat in second one if first and last is taken 
checkforgood(X,NF,NNF,_,M):-
	NF=\=1,
	N is 0,
	findgood(X,N,NNF,M).
checkforgood(_,NF,NNF,PM,M):-
	NF=:=1,
	NNF is 1,
	M is PM.
	
findgood([_],_,NNF,M):-M is -1, NNF is 0.
findgood([X,Y|_],N,NNF,M):-
	X = e,
	Y = e,
	NN is N+1,
	M is NN,
	NNF is 1.
findgood([X,Y|T],N,NNF,M):-
	X \= e,
	Y = e,
	NN is N+1,
...
checkbestright(X,NNF,L,_,M):-
	NNF=\=1,
	N is L,
	findbestright(X,N,M).
checkbestright(_,NNF,_,PM,M):-
	NNF=:=1,
	M is PM.

findbestright([],_,M):-M is -1.
findbestright([X|_],N,M):-
	X = e,
	NN is N-1,
	M is NN.
findbestright([X|T],N,M):-
	X \= e,
	NN is N-1,
